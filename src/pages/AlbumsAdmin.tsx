...
export default function AlbumsAdmin() {
  ...
  const { albums, loading, error, createAlbum, updateAlbum, deleteAlbum } = useAlbums();
  const { photos: albumFotos, loading: photosLoading, error: photosError, loadPhotos, createPhoto, updatePhoto, deletePhoto, clearPhotos } = usePhotos();
  ...
  }, [pendingBulkUpload, showBulkUpload, handleMultipleFileUpload, handleZipUpload]);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm(`Tem certeza que deseja excluir ${selectedPhotos.size} foto(s) selecionada(s)?`)) {
      try {
  ...
  return (
    <AdminLayout
      title={selectedAlbum ? `Álbum: ${selectedAlbum.titulo}` : 'Gerenciar Álbuns'}
      subtitle={selectedAlbum ? 'Gerencie as fotos deste álbum' : 'Administre seus álbuns de fotos'}
        ...
        {!selectedAlbum ? (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm p-8">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Álbuns de Fotos</h2>
                  <p className="text-gray-600 mt-1">Gerencie seus álbuns e organize suas fotos</p>
                </div>
                <button
            ...
            {albums.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-900">Nenhum álbum encontrado</h3>
                <p className="mt-2 text-gray-500">Comece criando seu primeiro álbum de fotos.</p>
              </div>
            ) : (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm p-8">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Fotos do Álbum</h2>
                  <p className="text-gray-600 mt-1">{albumFotos.length} foto(s) neste álbum</p>
                </div>
                  ...
                  {!selectionMode ? (
                    ...
                    <>
                      <span className="text-sm text-gray-600">
                        {selectedPhotos.size} de {albumFotos.length} selecionada(s)
                      </span>
                      <button
                        onClick={handleSelectAll}
                        className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors"
                      >
                        {selectedPhotos.size === albumFotos.length ? 'Desmarcar Todas' : 'Selecionar Todas'}
                      </button>
                      <button
            ...
            {albumFotos.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="mt-2 text-gray-500">Adicione a primeira foto a este álbum.</p>
              </div>
            ) : (
